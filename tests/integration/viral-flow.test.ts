...
async function cleanupViralTestData() {
  try {
    ...
    await testClient.from("viral_interactions").delete().gte("original_user_id", 500000);
...
Deno.test("Integration: Viral Flow - Complete user journey", async () => {
  ...
  const callbackQuery = {
    ...
    callback_query: {
      ...
      message: {
        ...
        text: `🎯 @originaluser, turn your ${merchantName} shopping into Max Miles!`,
      },
    },
  };
  ...
  if (viralInteraction) {
    ...
    console.log("⚠️ Viral interaction not tracked (may require valid merchant data)");
  }
  ...
  const { response: viralInlineResponse } = await callEdgeFunction(viralInlineQuery);
  assert(viralInlineResponse.status < 400, "Viral user inline query should succeed");
...
Deno.test("Integration: Viral Flow - Multiple viral users from same original", async () => {
  ...
  console.log(`✅ Multiple viral flow: ${viralUsers?.length || 0}/${viralUserIds.length} viral users processed`);
...
Deno.test("Integration: Viral Flow - Viral coefficient calculation", async () => {
  ...
  interactions.push(
    { original_user_id: originalUsers[0], viral_user_id: viralUsers[0], merchant_name: "Apple" },
    { original_user_id: originalUsers[0], viral_user_id: viralUsers[1], merchant_name: "Starbucks" },
    { original_user_id: originalUsers[0], viral_user_id: viralUsers[2], merchant_name: "Nike" },
  );
  
  // Original user 2 generates 1 viral user
  interactions.push(
    { original_user_id: originalUsers[1], viral_user_id: viralUsers[3], merchant_name: "Adidas" },
  );
  ...
  if (!interactionError && !userError && totalInteractions && totalOriginalUsers) {
    ...
    console.log("   - Expected coefficient:", interactions.length / originalUsers.length);
  } else {
    console.log("⚠️ Could not calculate viral coefficient due to data access issues");
  }
...
Deno.test("Integration: Viral Flow - Performance under viral load", async () => {
  ...
  for (let i = 0; i < viralUserCount; i++) {
    ...
    const callbackQuery = {
      ...
      callback_query: {
        ...
        data: `generate:${merchantName.toLowerCase().replace(/ /g, '-')}:${originalUserId}`,
        message: {
          message_id: 700 + i,
          date: Math.floor(Date.now() / 1000),
          chat: {
  ...
  console.log(`   - Average time per request: ${(totalTime / viralUserCount).toFixed(0)}ms`);
  
  // Performance assertions
  assert(totalTime < 10000, `Viral load should complete in <10s, took ${totalTime}ms`);
  assert(successCount >= viralUserCount * 0.8, `Success rate too low: ${successCount}/${viralUserCount}`);
