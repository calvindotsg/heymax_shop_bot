...
export async function cleanupAllTestData(): Promise<void> {
  ...
  try {
    ...
    await client.from("viral_interactions").delete().gte(
      "original_user_id",
      100000,
    );
...
export interface TestUser {
  ...
  total_viral_conversions: number;
}

export interface TestLinkGeneration {
  user_id: number;
...
export interface TestViralInteraction {
  original_user_id: number;
  viral_user_id: number;
  merchant_name: string;
...
export function generateTestLinkGeneration(
  userId: number,
  overrides: Partial<TestLinkGeneration> = {},
): TestLinkGeneration {
  return {
    user_id: userId,
...
export function generateTestViralInteraction(
  ...
  viralUserId: number,
  overrides: Partial<TestViralInteraction> = {},
): TestViralInteraction {
  return {
