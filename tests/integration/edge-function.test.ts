...
async function callEdgeFunction(payload: any, headers: Record<string, string> = {}) {
...
Deno.test("Integration: Edge Function - Inline query processing", async () => {
  ...
  assert(response.status >= 200 && response.status < 300, 
    `Expected 2xx status for inline query, got ${response.status}: ${JSON.stringify(data)}`);
  ...
  if (typeof data === 'object' && data.method === 'answerInlineQuery') {
    ...
    if (data.results.length > 0) {
      ...
      console.log("ℹ️ Inline query returned no results (may be expected for test query)");
    }
  }
...
Deno.test("Integration: Edge Function - Callback query processing", async () => {
  ...
  assert(response.status >= 200 && response.status < 300, 
    `Expected 2xx status for callback query, got ${response.status}: ${JSON.stringify(data)}`);
...
Deno.test("Integration: Edge Function - Response time performance", async () => {
  ...
  assert(response.status < 500, "Performance test should not cause server errors");
...
Deno.test("Integration: Edge Function - Concurrent request handling", async () => {
  ...
  assert(successCount >= concurrentRequests * 0.8, 
  ...
  console.log(`✅ Concurrent requests: ${successCount}/${concurrentRequests} succeeded in ${totalTime.toFixed(0)}ms`);
});
...
Deno.test("Integration: Edge Function - CORS headers", async () => {
  ...
  assert(response.status < 500, "Function should work with proper configuration");
