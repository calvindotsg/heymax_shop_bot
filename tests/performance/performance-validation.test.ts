import { assertEquals, assertExists, assert } from "testing/asserts.ts";
...
Deno.test("Performance - Bot should handle concurrent inline queries", async () => {
  ...
  assert(processingTime < 5000, `Concurrent processing should be under 5s, got ${processingTime}ms`);
  assert(avgResponseTime < 1000, `Average response time should be under 1s, got ${avgResponseTime}ms`);
});
...
Deno.test("Performance - Viral callback handling under load", async () => {
  ...
  // Validate responses
  responses.forEach((response, index) => {
    assertEquals(response.status, 200, `Viral callback ${index + 1} should succeed`);
  });

  // Performance validation
  assert(totalTime < 3000, `Viral callback processing should be under 3s, got ${totalTime}ms`);
});
...
Deno.test("Performance - Database query optimization", async () => {
  ...
  const analyticsRequest = await fetch('http://localhost:54321/functions/v1/telegram-bot/analytics', {
    method: 'GET',
    headers: { 'Content-Type': 'application/json' }
  });
  ...
  assert(queryTime < 2000, `Analytics query should be under 2s, got ${queryTime}ms`);
...
Deno.test("Performance - Memory usage validation", () => {
  ...
  const results = largeDataset
    .map(merchant => ({
      ...merchant,
      match_score: merchant.merchant_name.toLowerCase().includes(searchTerm) ? 0.8 : 0.3
    }))
  ...
  assert(memoryUsed < 10 * 1024 * 1024, `Memory usage should be reasonable, used ${memoryUsed} bytes`);
});
...
Deno.test("Performance - Response time consistency", async () => {
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    const response = await fetch('http://localhost:54321/functions/v1/telegram-bot', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        update_id: 2000 + i,
  ...
  console.log(`Response time stats: avg=${avgResponseTime.toFixed(2)}ms, max=${maxResponseTime}ms, min=${minResponseTime}ms, stddev=${stdDev.toFixed(2)}ms`);
  
  // Performance assertions
  assert(avgResponseTime < 500, `Average response time should be under 500ms, got ${avgResponseTime.toFixed(2)}ms`);
  assert(maxResponseTime < 2000, `Max response time should be under 2s, got ${maxResponseTime}ms`);
  assert(stdDev < 200, `Response time should be consistent (low std dev), got ${stdDev.toFixed(2)}ms`);
});
...
Deno.test("Performance - Viral coefficient calculation efficiency", async () => {
  ...
  const users = new Set([...mockViralData.map(d => d.original_user_id), ...mockViralData.map(d => d.viral_user_id)]);
  const viralCoefficient = mockViralData.length / users.size;
  
  const endTime = Date.now();
  const calculationTime = endTime - startTime;
  
  assert(calculationTime < 100, `Viral coefficient calculation should be fast, got ${calculationTime}ms`);
  assertEquals(typeof viralCoefficient, "number", "Viral coefficient should be numeric");
...
Deno.test("Performance - Free tier resource usage estimation", () => {
  ...
  const totalMonthlyInvocations = monthlyInlineQueries + monthlyViralCallbacks + monthlyStartCommands;
  const supabaseFreeLimit = 500000; // 500K invocations/month
  const usagePercentage = (totalMonthlyInvocations / supabaseFreeLimit) * 100;
  
  console.log(`Estimated monthly usage: ${totalMonthlyInvocations} invocations (${usagePercentage.toFixed(2)}% of free tier)`);
  
  assert(totalMonthlyInvocations < supabaseFreeLimit * 0.8, 
    `Usage should stay under 80% of free tier limit, estimated ${usagePercentage.toFixed(2)}%`);
  
  // Validate individual components
  assertEquals(typeof totalMonthlyInvocations, "number", "Total invocations should be numeric");
...
Deno.test("Performance - Error rate under load validation", async () => {
    ...
    switch (requestType) {
      case 0: // Inline query
        requestBody = {
          ...
          inline_query: {
            id: `load_test_${i}`,
            from: { id: 400000 + i, is_bot: false, first_name: "LoadTest", username: `load${i}` },
      ...
      case 1: // Callback query
        requestBody = {
          ...
          callback_query: {
            id: `callback_load_${i}`,
            from: { id: 500000 + i, is_bot: false, first_name: "CallbackTest", username: `cb${i}` },
            data: `generate:shopee-singapore:${400000 + i}`,
            chat_instance: `chat_${i}`,
            message: { message_id: i, chat: { id: -2000000 - i, type: "supergroup" } }
          }
        };
        break;
      case 2: // Start command
        requestBody = {
          ...
          message: {
            message_id: i,
            from: { id: 600000 + i, is_bot: false, first_name: "StartTest", username: `start${i}` },
  ...
  console.log(`Load test results: ${successCount} success, ${errorCount} errors, ${(errorRate * 100).toFixed(2)}% error rate`);
  
  assert(errorRate <= errorThreshold, 
    `Error rate should be under ${(errorThreshold * 100).toFixed(1)}%, got ${(errorRate * 100).toFixed(2)}%`);
  assert(successCount > totalRequests * 0.95, 
    `Should have >95% success rate, got ${(successCount / totalRequests * 100).toFixed(2)}%`);
});