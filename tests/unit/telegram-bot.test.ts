...
Deno.test("Merchant Search Logic - should handle empty search term", () => {
  ...
  assertEquals(searchTerm.length === 0, true, "Empty search term should be handled");
  assertStringIncludes(expectedBehavior, "top merchants", "Should return top merchants for empty search");
});
...
Deno.test("Merchant Search Logic - should handle search with term", () => {
  ...
  assertStringIncludes(expectedBehavior, "search by merchant name", "Should search by merchant name");
});

Deno.test("Link Personalization - should replace USER_ID placeholder", () => {
  const userId = 123456789;
  const templateLink = "https://shopee.sg/?utm_source=telegram&user_id={{USER_ID}}";
  const personalizedLink = templateLink.replace('{{USER_ID}}', userId.toString());
  
  assertEquals(personalizedLink.includes('{{USER_ID}}'), false, "Should replace USER_ID placeholder");
  assertStringIncludes(personalizedLink, userId.toString(), "Should contain actual user ID");
  assertEquals(personalizedLink, "https://shopee.sg/?utm_source=telegram&user_id=123456789", "Should generate correct personalized link");
});
...
Deno.test("Inline Query Result Structure - should have required fields", () => {
  const mockResult = {
    ...
    input_message_content: {
      message_text: "🛍️ **Shopee Singapore** - Earn up to **3.5 Max Miles per $1** spent\n\n*Exclusive link for @testuser* 👆",
      parse_mode: "Markdown" as const
    },
    reply_markup: {
      inline_keyboard: [
        [{ text: "🛍️ Shop Shopee Singapore & Earn Miles (for @testuser)", url: "https://shopee.sg/?utm_source=telegram&user_id=123456789" }],
        [{ text: "⚡ Get MY Unique Link", url: "https://t.me/HeyMax_shop_bot?start=shopee-sg" }]
      ]
    }
  };
  ...
  assertEquals(mockResult.reply_markup.inline_keyboard.length, 2, "Should have 2 button rows");
  assertEquals(mockResult.reply_markup.inline_keyboard[0].length, 1, "First row should have 1 button");
  assertEquals(mockResult.reply_markup.inline_keyboard[1].length, 1, "Second row should have 1 button");
});
...
Deno.test("User Data Structure - should validate user upsert data", () => {
  ...
  assertEquals(typeof upsertData.username, "string", "Username should be string");
  assertEquals(typeof upsertData.first_seen, "string", "Timestamp should be ISO string");
  assertStringIncludes(upsertData.first_seen, "T", "Should be valid ISO timestamp");
});
...
Deno.test("Link Generation Tracking - should validate tracking data structure", () => {
  ...
  assertEquals(typeof trackingData.user_id, "number", "User ID should be number");
  assertEquals(typeof trackingData.merchant_slug, "string", "Merchant slug should be string");
  assertExists(trackingData.generated_at, "Should have generation timestamp");
  assertStringIncludes(trackingData.unique_link, "telegram_", "Should have telegram prefix");
  assertStringIncludes(trackingData.unique_link, trackingData.user_id.toString(), "Should contain user ID");
});
...
Deno.test("Error Handling - should provide user-friendly error result", () => {
  ...
  assertEquals(errorResult.type, "article", "Error result should be article type");
  assertEquals(errorResult.id, "error", "Should have error id");
  assertStringIncludes(errorResult.title, "⚠️", "Should indicate error with warning emoji");
  assertStringIncludes(errorResult.description, "merchant name", "Should provide helpful guidance");
});
...
Deno.test("Response Time Validation - should process queries quickly", () => {
  ...
  assertEquals(processingTime < 1000, true, `Processing should be under 1000ms, got ${processingTime}ms`);
  assertEquals(processingTime >= 0, true, "Processing time should be non-negative");
});
...
Deno.test("Cache Configuration - should have appropriate cache settings", () => {
  ...
  assertEquals(cacheConfig.cache_time > 0, true, "Cache time should be positive");
});