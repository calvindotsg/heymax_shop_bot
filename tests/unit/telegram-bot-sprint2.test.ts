import { assertEquals, assertExists, assertStringIncludes } from "testing/asserts.ts";
...
Deno.test("Enhanced Inline Query - should handle empty query with popular merchants", () => {
  ...
  assertEquals(mockInlineQuery.query.trim().length, 0, "Empty query should be detected");
  assertExists(mockInlineQuery.from, "Should have user information");
  assertEquals(typeof mockInlineQuery.from.id, "number", "User ID should be number");
});

Deno.test("Enhanced Merchant Search - should calculate fuzzy match scores", () => {
  // Test fuzzy matching algorithm
  const testCases = [
    { merchantName: "Shopee Singapore", searchTerm: "shopee", expectedScore: 0.9 },
  ...
  testCases.forEach(({ merchantName, searchTerm, expectedScore }) => {
    ...
    assertEquals(actualScore >= expectedScore * 0.8, true, 
      `${merchantName} with search "${searchTerm}" should score at least ${expectedScore * 0.8}, got ${actualScore}`);
  });
});
...
Deno.test("Enhanced Link Generation - should include UTM parameters", () => {
  ...
  assertStringIncludes(finalLink, 'utm_source=telegram', "Should include telegram source");
  assertStringIncludes(finalLink, 'utm_campaign=viral_social_commerce', "Should include campaign");
  assertStringIncludes(finalLink, `utm_term=user_${userId}`, "Should include user term");
  assertStringIncludes(finalLink, `heymax_ref=telegram_${userId}`, "Should include HeyMax reference");
  assertEquals(finalLink.includes('{{USER_ID}}'), false, "Should replace placeholder");
});
...
Deno.test("Enhanced Bot Response - should generate engaging message format", () => {
  ...
  assertStringIncludes(response, displayName, "Should include user display name");
  assertStringIncludes(response, merchant.merchant_name, "Should include merchant name");
  assertStringIncludes(response, `${earnRate} Max Miles per $1`, "Should show earning rate");
  assertStringIncludes(response, `${exampleEarnings} Max Miles`, "Should show example earnings");
  assertStringIncludes(response, "Get MY Link", "Should include viral call-to-action");
});

Deno.test("Enhanced Viral Keyboard - should have proper button structure", () => {
  const userId = 123456789;
  const username = "testuser";
  const merchant = { merchant_name: "Shopee Singapore", merchant_slug: "shopee-singapore" };
  ...
  assertStringIncludes(keyboard.inline_keyboard[0][0].text, "🛍️ Shop", "First button should be shop action");
  assertStringIncludes(keyboard.inline_keyboard[1][0].text, "⚡ Get MY Unique Link", "Second button should be viral action");
  assertEquals((keyboard.inline_keyboard[0][0] as any).url, affiliateLink, "Shop button should have affiliate link");
  assertStringIncludes((keyboard.inline_keyboard[1][0] as any).callback_data, `generate:${merchant.merchant_slug}:${userId}`, 
...
Deno.test("No Results Response - should provide helpful suggestions", () => {
  ...
  assertStringIncludes(noResultsResult.title, searchTerm, "Should include search term in title");
  assertStringIncludes(noResultsResult.description, "popular brands", "Should suggest alternatives");
  assertStringIncludes(noResultsResult.input_message_content.message_text, "shopee, grab, klook", 
...
Deno.test("Analytics Tracking Structure - should validate enhanced tracking data", () => {
  ...
  assertEquals(typeof enhancedTracking.user_id, "number", "User ID should be number");
  assertEquals(typeof enhancedTracking.results_count, "number", "Results count should be number");
  assertStringIncludes(enhancedTracking.tracking_id, `tg_${userId}`, "Should include user in tracking ID");
  assertExists(enhancedTracking.search_term, "Should track search term");
  assertEquals(enhancedTracking.utm_source, "telegram", "Should track UTM source");
});

Deno.test("Popular Merchants Display - should show top earning opportunities", () => {
  const mockPopularMerchants = [
    { merchant_name: "Pelago by Singapore Airlines", base_mpd: 8.0, merchant_slug: "pelago" },
    { merchant_name: "Klook", base_mpd: 6.5, merchant_slug: "klook" },
    { merchant_name: "Shopee Singapore", base_mpd: 3.5, merchant_slug: "shopee-sg" }
  ];

  // Validate popular merchants are sorted by MPD
  const sortedByMPD = [...mockPopularMerchants].sort((a, b) => b.base_mpd - a.base_mpd);
  assertEquals(sortedByMPD[0].base_mpd >= sortedByMPD[1].base_mpd, true, "Should be sorted by MPD descending");
  assertEquals(sortedByMPD[1].base_mpd >= sortedByMPD[2].base_mpd, true, "Should maintain MPD order");
...
Deno.test("Error Handling - should provide enhanced error response", () => {
  ...
  assertStringIncludes(errorResult.title, "Temporary Service Issue", "Should indicate temporary nature");
  assertStringIncludes(errorResult.input_message_content.message_text, "• shopee • grab", 
    "Should provide merchant alternatives");
  assertStringIncludes(errorResult.input_message_content.message_text, "team has been notified", 
...
Deno.test("Enhanced Performance - should maintain response time under load", () => {
  ...
  assertEquals(processingTime < 1000, true, `Enhanced processing should be under 1000ms, got ${processingTime}ms`);
  assertEquals(processingTime < 500, true, `Should maintain performance under 500ms, got ${processingTime}ms`);
});
...
Deno.test("Match Scoring Performance - should calculate scores efficiently", () => {
  ...
  merchants.forEach(merchant => {
    ...
    assertEquals(score >= 0 && score <= 1, true, "Score should be between 0 and 1");
  });
  
  const processingTime = Date.now() - startTime;
  assertEquals(processingTime < 50, true, `Scoring should be fast, got ${processingTime}ms`);
});
...
Deno.test("Database Schema Validation - should support Sprint 2 enhancements", () => {
  ...
  const expectedColumns = {
    link_generations: ["tracking_id", "affiliate_link", "utm_source", "search_term", "results_count"],
    users: ["last_activity", "language_code", "search_count"],
    search_analytics: ["search_term", "results_count", "query_timestamp", "response_time_ms"]
  };
