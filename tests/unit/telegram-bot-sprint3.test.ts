import { assertEquals, assertExists, assertStringIncludes, assert } from "testing/asserts.ts";
...
Deno.test("Viral Callback Query - should handle valid callback data", () => {
  ...
  assertEquals(merchantSlug, "shopee-singapore", "Should extract merchant slug");
  assertEquals(originalUserId, "123456789", "Should extract original user ID");
  assertEquals(typeof mockCallbackQuery.from.id, "number", "User ID should be number");
});
...
Deno.test("Viral Interaction Tracking - should validate tracking data structure", () => {
  ...
  assertEquals(typeof viralTracking.original_user_id, "number", "Original user ID should be number");
  assertEquals(typeof viralTracking.viral_user_id, "number", "Viral user ID should be number");
  assertEquals(viralTracking.interaction_type, "callback_query", "Should track interaction type");
  assertExists(viralTracking.created_at, "Should have creation timestamp");
  assertStringIncludes(viralTracking.created_at, "T", "Should be valid ISO timestamp");
});
...
Need more help? Send /help anytime.`;
  ...
  assertStringIncludes(startCommandResponse, "Welcome to HeyMax Shop Bot!", "Should have welcome message");
  assertStringIncludes(startCommandResponse, "How to earn Max Miles", "Should explain earning process");
  assertStringIncludes(startCommandResponse, "Pelago", "Should list popular merchants");
  assertStringIncludes(startCommandResponse, "Viral earning", "Should explain viral mechanics");
  assertStringIncludes(startCommandResponse, "@HeyMax_shop_bot pelago", "Should provide example usage");
});
...
Ready to earn? Try @HeyMax_shop_bot pelago right now! 🚀`;
  ...
  assertStringIncludes(helpCommandResponse, "HeyMax Shop Bot Help", "Should have help header");
  assertStringIncludes(helpCommandResponse, "Basic Usage", "Should explain basic usage");
  assertStringIncludes(helpCommandResponse, "Viral Earning", "Should explain viral mechanics");
  assertStringIncludes(helpCommandResponse, "Pro Tips", "Should provide tips");
  assertStringIncludes(helpCommandResponse, "Issues?", "Should provide troubleshooting");
});
...
Try more: @HeyMax_shop_bot klook, pelago, grab...`;
  ...
  assertStringIncludes(viralResponse, displayName, "Should include user display name");
  assertStringIncludes(viralResponse, "viral", "Should emphasize viral nature");
  assertStringIncludes(viralResponse, `${exampleEarnings} Max Miles`, "Should show calculated earnings");
  assertStringIncludes(viralResponse, "viral sharing", "Should reference viral discovery");
  assertStringIncludes(viralResponse, "Keep the viral loop going", "Should encourage continued sharing");
});
...
Deno.test("Analytics Summary Structure - should provide comprehensive metrics", () => {
  ...
  assertExists(mockAnalytics.performance_metrics, "Should have performance metrics");
  assertExists(mockAnalytics.health_status, "Should have health status");

  // Validate user metrics
  assertEquals(typeof mockAnalytics.user_metrics.total_users, "number", "Total users should be number");
  assertEquals(typeof mockAnalytics.user_metrics.active_users_7d, "number", "Active users should be number");

  // Validate viral metrics
  assertEquals(typeof mockAnalytics.viral_metrics.viral_coefficient_7d, "number", "Viral coefficient should be number");
  assert(mockAnalytics.viral_metrics.viral_coefficient_7d > 1, "Should show viral growth (>1.0 coefficient)");

  // Validate performance metrics
  assert(mockAnalytics.performance_metrics.avg_response_time_ms < 1000, "Response time should be under 1s");
  assert(mockAnalytics.performance_metrics.uptime_percentage > 99, "Uptime should be >99%");
  assert(mockAnalytics.performance_metrics.error_rate_percentage < 1, "Error rate should be <1%");

  // Validate health status
  assertEquals(mockAnalytics.health_status.overall_status, "operational", "Should be operational");
});
...
Deno.test("Viral Coefficient Calculation - should calculate correctly", () => {
  ...
  assertEquals(typeof viralCoefficient, "number", "Viral coefficient should be number");
  ...
  assertEquals(noViralCase, 0, "No viral interactions should result in 0 coefficient");
  
  const perfectViralCase = 100 / 100;
  assertEquals(perfectViralCase, 1.0, "Equal interactions should result in 1.0 coefficient");
});
...
Deno.test("Function Invocation Monitoring - should track usage limits", () => {
  ...
  assertEquals(typeof mockUsageData.monthly_invocations, "number", "Invocations should be number");
  assertEquals(typeof mockUsageData.usage_percentage, "number", "Usage percentage should be number");
  assertEquals(mockUsageData.warning_threshold_reached, false, "Should not have reached warning threshold");
  assertEquals(mockUsageData.critical_threshold_reached, false, "Should not have reached critical threshold");
  ...
  assert(criticalUsage.usage_percentage > 95, "Should trigger critical alert at >95%");
});
...
Deno.test("Top Merchants Ranking - should rank by activity", () => {
  ...
  // Validate ranking
  assertEquals(topMerchantsRanked[0].merchant, "shopee-singapore", "Shopee should be #1");
  assertEquals(topMerchantsRanked[0].count, 3, "Shopee should have 3 occurrences");
  assertEquals(topMerchantsRanked[1].merchant, "klook", "Klook should be #2");
  assertEquals(topMerchantsRanked[1].count, 3, "Klook should have 3 occurrences");
...
Deno.test("Performance Metrics Validation - should meet production standards", () => {
  ...
  assert(performanceMetrics.avg_response_time_ms < 1000, "Average response time should be <1000ms");
  assert(performanceMetrics.p95_response_time_ms < 2000, "95th percentile should be <2000ms");
  assertEquals(performanceMetrics.performance_target_met, true, "Should meet performance targets");
  assertEquals(performanceMetrics.health_status, "healthy", "Should report healthy status");
  
  // Validate request volume
  assertEquals(typeof performanceMetrics.total_requests_1h, "number", "Request count should be number");
  assert(performanceMetrics.total_requests_1h > 0, "Should have positive request count");
});
...
Deno.test("Production Health Check - should validate all systems", () => {
  ...
  assertEquals(healthCheck.edge_function.status, "healthy", "Edge function should be healthy");
  assertEquals(healthCheck.edge_function.response_code, 200, "Should return 200 status");
  assert(healthCheck.edge_function.response_time_ms < 1000, "Response time should be reasonable");

  // Validate database health
  assertEquals(healthCheck.database.status, "healthy", "Database should be healthy");
  assertEquals(healthCheck.database.connection, "active", "Database connection should be active");
  assert(healthCheck.database.query_time_ms < 1000, "Database queries should be fast");

  // Validate Telegram API health
  assertEquals(healthCheck.telegram_api.status, "healthy", "Telegram API should be healthy");
  assertEquals(healthCheck.telegram_api.webhook_configured, true, "Webhook should be configured");
  assertEquals(healthCheck.telegram_api.pending_updates, 0, "Should have no pending updates");

  // Validate overall status
  assertEquals(healthCheck.overall_status, "operational", "Overall status should be operational");
});
...
Deno.test("Viral Growth Trend Analysis - should track daily coefficients", () => {
  const mockViralTrend = [
    ...
    { date: "2025-08-24", coefficient: 1.4, users: 80, viral_interactions: 112 },
    { date: "2025-08-25", coefficient: 1.5, users: 90, viral_interactions: 135 }
  ];
  ...
  mockViralTrend.forEach((day, index) => {
    ...
    assertEquals(typeof day.coefficient, "number", `Day ${index + 1} coefficient should be number`);
    assertEquals(typeof day.users, "number", `Day ${index + 1} users should be number`);
    assertEquals(typeof day.viral_interactions, "number", `Day ${index + 1} interactions should be number`);
  });
  ...
  assert(lastDay.coefficient > firstDay.coefficient, "Should show improving viral coefficient");
  assert(lastDay.users > firstDay.users, "Should show user growth");
  assert(lastDay.viral_interactions > firstDay.viral_interactions, "Should show interaction growth");
  
  // Check viral threshold achievement
  const viralDays = mockViralTrend.filter(day => day.coefficient > 1.0);
  assert(viralDays.length >= 3, "Should achieve viral growth (>1.0) for multiple days");
});
...
Deno.test("Complete Viral Flow - should work end-to-end", () => {
  ...
  const merchant = { merchant_slug: "shopee-singapore", merchant_name: "Shopee Singapore", base_mpd: 3.5 };
  ...
  assertEquals(merchantSlug, merchant.merchant_slug, "Should parse merchant correctly");
  assertEquals(parseInt(originalUserId), originalUser.id, "Should parse original user ID correctly");
  ...
  assertEquals(viralInteraction.original_user_id, originalUser.id, "Should track original user");
  assertEquals(viralInteraction.viral_user_id, viralUser.id, "Should track viral user");
  assertEquals(viralInteraction.merchant_slug, merchantSlug, "Should track merchant");
  ...
  assert(afterCoefficient > beforeCoefficient, "Viral coefficient should increase");
