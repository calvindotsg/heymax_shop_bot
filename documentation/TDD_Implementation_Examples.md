...
_Red-Green-Refactor Examples for Key Features_
...
Deno.test("InlineQuery: handleInlineQuery_EmptyQuery_ReturnsHelpMessage", async () => {
  ...
  assert(
    result.results[0].description.includes("Try: @HeyMax_shop_bot pelago"),
  );
});
      ...
      results: [
        {
          ...
          input_message_content: {
            message_text:
              "💡 Type merchant merchant_name after @HeyMax_shop_bot to find earning opportunities!",
          },
        },
      ],
    };
      ...
      results: [
        {
          ...
          input_message_content: {
            message_text:
              `🔍 No merchants found for "${query}". Try popular brands!`,
          },
        },
      ],
    };
    ...
    results: [
      {
        ...
        title: "🔍 Search for merchants",
        description:
          "Type a merchant merchant_name to find earning opportunities",
        input_message_content: {
          message_text:
            "💡 Type merchant merchant_name after @HeyMax_shop_bot to find earning opportunities!\n\n" +
  ...
  try {
    ...
    const results = await Promise.all(
      merchants.map((merchant) =>
        createMerchantResult(merchant, userId, username)
      ),
    );
    ...
    results: [
      {
        ...
        input_message_content: {
          message_text:
            "⚠️ We encountered a temporary issue. Please try your search again!",
        },
      },
    ],
...
} from "../../../src/services/affiliate-links.ts";
...
Deno.test("AffiliateLinks: generateAffiliateLink_ValidInput_ReturnsPersonalizedLink", async () => {
  ...
  assert(
    link.includes("utm_campaign=heymax_shop_bot"),
    "Should include UTM campaign",
  );
});
...
Deno.test("AffiliateLinks: generateAffiliateLink_ValidInput_CreatesTrackingRecord", async () => {
  ...
  await generateAffiliateLink(userId, merchantSlug, testClient, {
    chatId,
    trackingEnabled: true,
  });
...
export async function generateAffiliateLink(
  ...
  try {
    ...
    console.error(
      `Affiliate link generation failed for user ${userId}, merchant ${merchantSlug}:`,
      error,
    );
...
export async function generateAffiliateLinkWithDetails(
  ...
  if (options.trackingEnabled !== false) {
    trackingId = await createTrackingRecord(
      userId,
      merchantSlug,
      options.chatId || null,
      supabaseClient,
...
export async function handleCallbackQuery(
  ...
  try {
    ...
    const { merchantSlug, originalUserId } = parseAndValidateCallbackData(
      callbackQuery.data,
    );
...
export async function handleCallbackQueryBatch(
  ...
  for (let i = 0; i < callbackQueries.length; i += batchSize) {
    ...
    const batchResults = await Promise.allSettled(
      batch.map((query) =>
        handleCallbackQuery(query, supabaseClient, telegramClient)
      ),
    );
...
3. **Refactor with Confidence**: Improve code knowing tests will catch
   regressions
4. **Test Behavior, Not Implementation**: Focus on what the code should do, not
   how
5. **One Test, One Behavior**: Each test should verify a single aspect of
   behavior
...
This comprehensive TDD implementation guide provides practical examples of how
to apply Test-Driven Development principles to the HeyMax_shop_bot project,
ensuring reliable, maintainable, and well-tested code throughout the development
process.
